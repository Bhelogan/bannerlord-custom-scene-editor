...
> **Note (2026-08-08):** Move mode's carry/target tracking is the foundation for this. It already
> resolves a raycast hit back to the object that owns it, holds a selection across an interaction,
> and reports it in the status readout - which is exactly the "pick an object, then act on it"
> loop script attachment needs. Attaching a fire to a brazier should reuse that selection rather
> than inventing a second way to point at an object.
